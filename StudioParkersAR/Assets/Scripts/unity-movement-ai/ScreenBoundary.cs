...
public class ScreenBoundary : MonoBehaviour {
    ...
    void Start () {
        ...
        Debug.Log("widthHeight: " + widthHeight);
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        Debug.Log("onTriggerStay: ball position x: " + t.position.x);
        if (t.position.x < bottomLeft.x)
        {
            Debug.Log("Status Report:: OnTriggerEnter:: t.position.x < bottomLeft.x");
            t.position = new Vector3(t.position.x + widthHeight.x, t.position.y, t.position.z);
        }
        if (t.position.x > topRight.x)
        {
            Debug.Log("Status Report:: OnTriggerEnter:: t.position.x > topRight.x");
            t.position = new Vector3(t.position.x - widthHeight.x, t.position.y, t.position.z);
        }

        if (t.position.y <  bottomLeft.y)
        {
            Debug.Log("Status Report:: OnTriggerEnter:: t.position.y < bottomLeft.y");
            t.position = new Vector3(t.position.x, t.position.y + widthHeight.y, t.position.z);
        }
        if (t.position.y > topRight.y)
        {

            Debug.LogError("Activate Self-Destruct Sequens");
            Debug.Log("Status Report:: OnTriggerEnter:: t.position.y > topRight.y");
            t.position = new Vector3(t.position.x, t.position.y - widthHeight.y, t.position.z);
        }
    }
    ...
    {
        Debug.Log("Status Report:: OnTriggerExit:: Trying To Escape");
        Transform t = other.transform;
